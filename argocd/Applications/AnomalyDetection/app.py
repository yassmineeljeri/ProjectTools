...
MODEL_PATH = os.getenv("MODEL_PATH", "/app/xgb_model.pkl")
...
def fetch_loki_logs():
    ...
    try:
        ...
        start = (int(pd.Timestamp.utcnow().timestamp()) - 300) * 1_000_000_000  # last 5 min
...
def predict_from_local_json():
    ...
    try:
        ...
        # ---------------------------------------------
        # 🧩 3) SEND MALICIOUS TRAFFIC ALERTS (same logic)
        # ---------------------------------------------
        alert_count = 0
        pred = 0
